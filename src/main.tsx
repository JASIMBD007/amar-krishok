  ...
  return (
    <section className="dashboard-shell">
      <aside className="sidebar">
        ...
        {["Orders", "Supply Lots", "Farmers", "Logistics", "Payouts"].map((item, index) => (
          <button className={index === 0 ? "active" : ""} type="button" key={item}>{item}</button>
        ))}
      </aside>
      <div className="workspace">
        <SectionTitle eyebrow="Admin dashboard" title="Manage orders, verification, logistics, and payouts." />
        <div className="stats-grid">
          <StatCard icon={Banknote} label="GMV today" value="৳4.82L" detail="18 orders confirmed" />
          <StatCard icon={Sprout} label="Active supply" value="27.4 tons" detail="63 verified lots" />
          <StatCard icon={HandCoins} label="Payout queue" value="৳82K" detail="3 orders awaiting QC" />
          <StatCard icon={Truck} label="Routes active" value="7" detail="2 delayed pickups" />
        </div>
        <div className="panel orders-panel">
          <div className="panel-header">
            <div>
              <span>Order control</span>
              <h2>Buyer demand queue</h2>
            </div>
              ...
              <tbody>
                {orders.map((order) => (
                  <tr key={order.id}>
                    ...
                    <td>{order.value}</td>
                    <td><em className="status-pill small">{order.status}</em></td>
                  </tr>
                ))}
              </tbody>
            </table>
